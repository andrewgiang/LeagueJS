...
(function () {
    ...
    League.getLeagueChallenger = function (type, regionOrFunction, callback) {
        var regionAndFunc = util.getCallbackAndRegion(regionOrFunction, region, callback),
            url = util.craftUrl(endpoint, regionAndFunc.region, leagueUrl.challenger + '/?' + 'type=' + type + '&', authKey);

        return util.makeRequest(url, 'Error getting league data: ', null, regionAndFunc.callback);
    };

    League.getLeagueMaster = function (type, regionOrFunction, callback) {
        var regionAndFunc = util.getCallbackAndRegion(regionOrFunction, region, callback),
            url = util.craftUrl(endpoint, regionAndFunc.region, leagueUrl.master + '/?' + 'type=' + type + '&', authKey);
